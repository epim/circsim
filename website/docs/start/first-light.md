...
Each lead you drew told circsim which nets the pot connects. In rheostat mode it emits a single resistor of `totalR × wiperPct` between A and W. Turning the wiper knob issues a live *alter* (an in-place value change the SPICE engine applies without rebuilding the circuit) — no restart, no reload. This is the same mechanism the supply knob uses.
:::
