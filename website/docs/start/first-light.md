...
At 5 V through 330 Ω into a red LED (about 1.8 V of *forward voltage* dropped when conducting), the current is roughly **(5 − 1.8) / 330 ≈ 9.7 mA**, a healthy, clearly-lit LED. Hover the parts to read the numbers back.
...
3. **Draw the leads.** The pot has two terminals here: **A** (one end of the resistive track) and **W** (the wiper, the sliding tap you're controlling). Press and hold the **A** jack; a wire follows your cursor. Drag it up onto the board and drop it on the supply-side copper (the net between the supply and R1). Then draw the **W** lead onto the LED-side net. As you drag, circsim highlights the net you're about to land on.
...
Each lead you drew told circsim which nets the pot connects. In rheostat mode it emits a single resistor of `totalR × wiperPct` between A and W. Turning the wiper knob issues a live *alter* (an in-place value change the SPICE engine applies without rebuilding the circuit). It doesn't restart or reload. This is the same mechanism the supply knob uses.
:::
...
1. Press **`Run`**: circsim streams a live **transient** simulation (a transient shows voltages changing over *time*, versus the operating point's single frozen instant).
2. Click the LED-side net, then **`⌖ Probe this net`** in the bench header. The **scope** in the bottom dock (an oscilloscope, which draws voltage over time) starts tracing that net's voltage. (Never used a scope? The **Time/div** control just zooms the time axis in or out; leave it for now.)
