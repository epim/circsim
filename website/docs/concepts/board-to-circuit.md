...
KiCad 6–8 boards carry a numeric net table — `(net 3 "VCC")` — and tag pads by id. KiCad 9 (and the 2026 format) dropped the numeric ids entirely: nets are referenced by **name only**. circsim reads both. For name-only boards it synthesizes a stable internal id per distinct net name, so everything downstream is identical regardless of which KiCad version routed the board. It also handles the older `F.SilkS` and newer `F.Silkscreen` layer spellings.
:::
