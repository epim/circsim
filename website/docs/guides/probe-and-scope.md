...
The transient starts from the circuit's initial state rather than a pre-solved DC point, so you watch capacitors charge and oscillators start up — the "power on and see it come alive" moment. For very long runs, circsim restarts the window every ~30 seconds to bound memory, keeping your scope history.
:::
...
- Nothing moving? A transient needs a *changing* source — a [function generator](./bench-and-leads#function-generator--func-gen), an astable oscillator, or a logic input you toggle. A purely DC circuit is a flat line (that's correct — use the operating point for DC).
