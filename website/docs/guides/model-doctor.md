...
Each problem part shows a status pill — **no model** (red), **stubbed** (amber), or **open by design** (grey) — its reference, value, and library id, plus any warnings (an ambiguous match, an unverified pin order, an electrolytic-polarity caution). Below that is a row of actions.
