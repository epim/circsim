...
| --- | --- | --- |
| **Volts** (knob) | 0–30 V, step 0.1 | Same value as the Voltage field |
| **Voltage** (field) | 0–30 V | Type an exact value |
...
| **Freq** (knob) | 1 Hz – 1 MHz, logarithmic | Log scale gives fine low-frequency control |
| **Amp** (knob) | 0–20 V | Amplitude |
| **Offset** (knob) | ±20 V | DC offset |
| **Duty** (knob) | 1–99 % | Square/pulse only |
...
| **V High** | 0–30 V, default 3.3 V | Set it to match your logic rail |
...
| **Wiper** (knob) | 0–100 % | Wiper position |
...
- **`alter` (instant, in place)** — changing a *value*: supply voltage, logic level, function-gen amplitude/frequency/offset (same wave), pot wiper.
- **Reload (rebuild the netlist)** — changing *topology*: moving a lead to a different net, changing a function-gen wave type, changing pot mode or Total R, or adding/removing a current probe.
