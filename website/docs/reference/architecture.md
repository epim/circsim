...
Crucially, ngspice runs in a **separate, isolated process** (an Electron utility process), talking to the app over a message channel. This matters for reliability:
...
- `src/core/` — framework-free logic: KiCad parsing, netlist extraction, model resolution, SPICE-deck generation, the Board Critic. Fully unit-tested.
- `src/simhost/` — the isolated ngspice process and its koffi FFI bindings.
- `src/renderer/` — the React UI, the zustand store, the Three.js viewport, and the bench.
- `src/main/` — the Electron main process.
- `resources/models/` — the bundled SPICE model library and its index.
