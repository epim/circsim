...
| 3 | **Floor + validity.** Tier 3 applies only if measured rail `> 2 V`, finite, and `< ~30 V` sanity cap. Skip pass 2 if no chip's `vHigh` changed (> 0.1 V epsilon). | Guards against deriving a dead ~0 V rail (see #4) or an absurd value from a marginal op; avoids a needless second solve. |
